...
/**     commercially. Please, notify me, if you make asny    **/
...
void OutZ80(register word16 Port, register byte Value)
{
  if ((Port & 0xFF) == 0xFE) 
  {
    ...
    //    out_ram=Value;
    // Ton-Signal schalten
    zxDisplayBorderSet(Value & 0x07);

    //0x10 = speaker (EAR), 0x08 = tape(MIC)
/*    if ( Value & 0x10 )
    //if ( Value & 0x18 ) //produce a sound even when we are saving :) this needs to be an option as interferes with some games
    {
      zxSoundSet(1);
    }
    else {
...
"If Port 0xfe is read from, the highest eight address lines are important too. A zero on one of these lines selects a particular half-row of five keys:"
 ...
 from https://worldofspectrum.org/faq/reference/48kreference.htm
*/
  if ((Port & 0xFF) == 0xFE) 
  {
    ...
    //TAP LOADING: set bit 6(0x40) from EAR input (audio)
    // Need some condition to only do it when we are trying to load
    //test to simulate a sound: later from .tap files :)
    //if ((micros() & 0x3FF) < 0x1FF)
    //    ret_wert &= ~0x20;
  }
...
/*************************************************************/
void ICACHE_FLASH_ATTR ResetZ80(Z80 *R)
{
  R->PC.W     = 0x0000;
  R->SP.W     = 0xF000;
  R->AF.W     = 0x0000;
...
void RetnZ80(Z80 *R)
{
    ...
    R->PC.B.l=OpZ80(R->SP.W++); R->PC.B.h=OpZ80(R->SP.W++); JumpZ80(R->PC.W)          
}
...
int ExecZ80(register Z80 *R, register int RunCycles)
{
  ...
  for (R->ICount = RunCycles;;)
  {
    ...
    //z80 engine too fast at 160mhz. let's burn some cycle to slowdown
    //this would be nicer with delayMicroseconds to use less power and conserve battery
    //doesn't seem to work though! why?
